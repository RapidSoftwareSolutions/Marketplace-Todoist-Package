...
$app->post('/api/Todoist/completeRecurringTask', function ($request, $response) {
    ...
    foreach( $data['commands'] as $key => $value)
    {
        ...
        if(!empty($value['is_forward']))
        {
            if($value['is_forward'] == 'true')
            {
                $value['is_forward'] = 1;

            } else if($value['is_forward'] == 'false')
            {
                $value['is_forward'] = 0;
            }
        }

        if(!empty($value['new_date_utc']))
        {
            $value['new_date_utc'] =   date('Y-m-d', strtotime($value['new_date_utc'])).'T'.date('H:i', strtotime($value['new_date_utc']));
