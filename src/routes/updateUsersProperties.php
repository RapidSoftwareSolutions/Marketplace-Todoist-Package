...
$app->post('/api/Todoist/updateUsersProperties', function ($request, $response) {
    ...
    foreach( $data['commands'] as $key => $value)
    {
        ...
        if(!empty($value['date_format']))
        {
            if($value['date_format'] == 'DD-MM-YYYY')
            {
                $value['date_format'] = 0;
            } else {
                $value['date_format'] = 1;
        ...
        {
            if($value['sort_order'] == 'oldestDatesFirst')
            {
                $value['sort_order'] = 0;
            } else {
                $value['sort_order'] = 1;
